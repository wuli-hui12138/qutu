...
export default function MobileLayout() {
    ...
    const hideNavPaths = ['/upload', '/detail', '/settings', '/ai'];
            ...
            <div className="w-full max-w-[375px] bg-gray-50 flex flex-col shadow-2xl h-full overflow-hidden relative">
                ...
                {!hideNav && (
                    <div className="fixed bottom-0 w-full max-w-[375px] h-20 bg-white/80 backdrop-blur-md border-t border-gray-100 flex items-start pt-3 justify-around z-40 px-4">
                        ...
                        <NavLink to="/ai" className={({ isActive }) => clsx("flex flex-col items-center transition-colors", isActive ? "text-purple-600" : "text-gray-400")}>
                            {({ isActive }) => (
                                <>
                                    <Sparkles size={24} strokeWidth={isActive ? 2.5 : 2} />
                                    <span className="text-[10px] mt-1">AI中心</span>
                                </>
                            )}
                        </NavLink>
